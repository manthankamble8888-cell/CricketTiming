...
public class OverlayService extends Service {
    ...
    //
    // Mimics Apple's "Liquid Glass" material on the card itself only —
    // a translucent vertical tint, a hairline light border, and a
    // brighter specular highlight along the top edge. The rest of the
    // screen behind the overlay stays fully sharp (no FLAG_BLUR_BEHIND).
    // ============================================================
    ...
    private void createOverlay() {
        ...
        // ========================================================
        // (Apply Target button removed — presets + the keyboard's
        // Done key now apply the typed value; see the target input
        // listeners below.)
        // ========================================================
        ...
        // ========================================================
        // APPLY TARGET (via keyboard Done key / losing focus)
        // ========================================================

        targetInput.setOnEditorActionListener(
                (v, actionId, event) -> {
    ...
    // TOGGLE MINIMIZE
    //
    // Called as a "tap" (not a drag) on the minimize button, from
    // makeDraggable's click-vs-drag detection. Kept as its own
    // method so the minimize button can stay draggable (see
    // makeDraggable) while still supporting a plain tap to toggle.
    // ============================================================
...
private void setSmallButtonHeight(
    ...
    private void makeDraggable(
        ...
        makeDraggable(dragView, overlayView, params, null);
    }

    // Same as above, but also supports a tap action (e.g. the
    // minimize button, which needs to stay both draggable AND
    // tappable — including while minimized, when it's the only
    // visible view left to grab).
    private void makeDraggable(
            View dragView,
            View overlayView,
            WindowManager.LayoutParams params,
        ...
        // Small movement threshold (in px) below which a touch is
        // treated as a tap rather than a drag.
        final int tapSlop = 12;
    ...
    // START TIMING
    //
    // Shared by the START button and by tapping a preset — tapping
    // a saved preset applies its target AND kicks off the timer
    // immediately, so there's no separate START tap needed.
    // ============================================================
